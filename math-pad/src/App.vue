...
export default {
  ...
  setup(_, { root }) {
    ...
    const routeParams = computed(() => {
      ...
      if(params.get('pad')) {
        console.log(params.get('pad'))
        fetch(`https://short.colehollant.com/api/text/${params.get('pad')}`)
          .then(res => res.text())
          .then(res => {
    ...
    const pushRoute = (query) => {
      if (history.pushState) {
        const pad = query.pad ? ['pad', query.pad].join('=') : null
        const layout = query.layout ? ['layout', query.layout].join('=') : null
        var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname + `?${[pad, layout].filter(a => a !== null).join('&')}`;
        window.history.pushState({path:newurl},'',newurl);
      }
    }

    const pushExample = () => {
      pushRoute({ pad: 'example' })
    }
    ...
    const setLayout = (key) => {
      layout.value = key
      if(routeParams.value.layout !== key) {
        // root.$router.push({ path: '/', query: { ...routeParams.value, layout: layout.value }})
        pushRoute({ pad: routeParams.value.pad, layout: layout.value })
      }
      rerender()
    }
    ...
    watch(responseState, () => {
      if(responseState.result && responseState.result.short_id) {
        console.log(root)
        if(routeParams.value.pad && routeParams.value.pad === responseState.result.short_id && routeParams.value.layout === layout.value) return
        // else root.$router.push({ path: '/', query: { pad: responseState.result.short_id, layout: layout.value }})
        else pushRoute({ pad: responseState.result.short_id, layout: routeParams.value.layout })
      }
    })

    // watchEffect(() => {
    //   if(routeParams.value.pad) {
    //     console.log(routeParams.value.pad)
    //     fetch(`https://short.colehollant.com/api/text/${routeParams.value.pad}`)
    //       .then(res => res.text())
    //       .then(res => {
    //         if(res != formula.value) {
    //           formula.value = res
    ...
    // })
    // watch(routeParams, () => {
    //   console.log('routeparams updated')
    //   if(routeParams.value.pad) {
    //     console.log(routeParams.value.pad)
    //     fetch(`https://short.colehollant.com/api/text/${routeParams.value.pad}`)
    //       .then(res => res.text())
    ...
    onMounted(() => {
      if(routeParams.value.layout) {
        layout.value = routeParams.value.layout
      }
      if(!routeParams.value.pad) {
        setFormula(sample)
      }
